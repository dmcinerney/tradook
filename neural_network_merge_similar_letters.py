...
def accuracy(label, output): # function to calculate accuracy by comparing the labels to the output of the network
    ...
    for i in range(0, y):
        ...
        if (label_numpy == 0 or label_numpy == 24): # merge 0, O and o
            label_numpy = 50
        if (letter == 0 or letter == 24):
            letter = 50
            
        if (label_numpy == 32): # merge w and W
            label_numpy = 58
        if (letter == 32):
            letter = 58

        if (label_numpy == 25): # merge p and P
            label_numpy = 51
        if (letter == 25):
            letter = 51

        if (label_numpy == 12): # merge c and C
            label_numpy = 38
        if (letter == 12):
            letter = 38

        if (label_numpy == 28): # merge s and S
            label_numpy = 54
        if (letter == 28):
            letter = 54

        if (label_numpy == 20): # merge k and K
            label_numpy = 46
        if (letter == 20):
            letter = 46

        if (label_numpy == 49): # merge n and N
            label_numpy = 23
        if (letter == 49):
            letter = 23

        if (label_numpy == 22): # merge m and M
            label_numpy = 48
        if (letter == 22):
            letter = 48
